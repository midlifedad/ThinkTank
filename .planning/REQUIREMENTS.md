...
- [ ] **PIPE-01**: Operator can view the job queue with filters by status (pending, running, failed, complete), job type, and date range
- [ ] **PIPE-02**: Operator can manually trigger pipeline jobs: refresh_due_sources, scan_for_candidates, discover_guests for a thinker
- [ ] **PIPE-03**: Operator can configure recurring task schedules with frequency (hours), enable/disable toggle, and a Run Now button
- [ ] **PIPE-04**: Operator can retry a failed job or cancel a pending job from the queue view
- [ ] **PIPE-05**: Operator can view job detail showing payload, attempts, error messages, and timing
