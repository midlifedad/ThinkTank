...
async def test_due_source_gets_job(session: AsyncSession):
    ...
    trigger_job = await create_job(session, job_type="refresh_due_sources", payload={})
    await session.commit()

    await handle_refresh_due_sources(session, trigger_job)

    result = await session.execute(
        select(Job).where(Job.job_type == "fetch_podcast_feed", Job.payload["source_id"].astext == str(source.id))
    )
...
async def test_never_fetched_source_due(session: AsyncSession):
    ...
    await create_thinker(session)
    source = await create_source(
        session, approval_status="approved", active=True, refresh_interval_hours=24, last_fetched=None
    )
    trigger_job = await create_job(session, job_type="refresh_due_sources", payload={})
    await session.commit()
...
async def test_unapproved_source_not_due(session: AsyncSession):
    ...
    await create_thinker(session)
    await create_source(
        session, approval_status="pending_llm", active=True, refresh_interval_hours=24, last_fetched=None
    )
    trigger_job = await create_job(session, job_type="refresh_due_sources", payload={})
    await session.commit()
...
async def test_inactive_source_not_due(session: AsyncSession):
    ...
    await create_thinker(session)
    await create_source(session, approval_status="approved", active=False, refresh_interval_hours=24, last_fetched=None)
    trigger_job = await create_job(session, job_type="refresh_due_sources", payload={})
    await session.commit()
...
async def test_orchestrator_creates_jobs(session: AsyncSession):
    ...
    source2 = await create_source(
        session, approval_status="approved", active=True, refresh_interval_hours=6, last_fetched=None
    )
...
async def test_existing_inflight_job_dedup(session: AsyncSession):
    ...
    await create_job(session, job_type="fetch_podcast_feed", payload={"source_id": str(source.id)}, status="pending")
    trigger_job = await create_job(session, job_type="refresh_due_sources", payload={})
    await session.commit()
...
async def test_youtube_source_gets_youtube_job(session: AsyncSession):
    ...
    result = await session.execute(
        select(Job).where(Job.job_type == "fetch_youtube_channel", Job.payload["source_id"].astext == str(source.id))
    )
