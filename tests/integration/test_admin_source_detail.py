...
class TestSourceDetail:
    ...
    async def test_detail_page_loads(self, admin_client, session: AsyncSession):
        ...
        source = await create_source(session, name="Detail Source Feed", url="https://example.com/detail-feed.xml")
    ...
    async def test_detail_shows_health_info(self, admin_client, session: AsyncSession):
        ...
        await create_thinker(session, name="Health Thinker", slug="health-thinker")
        source = await create_source(
            session, name="Health Source", url="https://example.com/health-feed.xml", error_count=3, item_count=10
        )
        await session.commit()
...
class TestSourceEpisodes:
    ...
    async def test_episodes_partial_empty(self, admin_client, session: AsyncSession):
        ...
        await create_thinker(session, name="No Eps Thinker", slug="no-eps-thinker")
        source = await create_source(session, name="No Eps Source", url="https://example.com/no-eps-feed.xml")
        await session.commit()
    ...
    async def test_episodes_partial_shows_content(self, admin_client, session: AsyncSession):
        ...
        await create_thinker(session, name="Eps Thinker", slug="eps-thinker")
        source = await create_source(session, name="Eps Source", url="https://example.com/eps-feed.xml")
        await create_content(session, source_id=source.id, title="Episode Alpha")
        await create_content(session, source_id=source.id, title="Episode Beta")
        await session.commit()
...
class TestSourceErrors:
    ...
    async def test_errors_partial_empty(self, admin_client, session: AsyncSession):
        ...
        await create_thinker(session, name="No Errs Thinker", slug="no-errs-thinker")
        source = await create_source(session, name="No Errs Source", url="https://example.com/no-errs-feed.xml")
        await session.commit()
    ...
    async def test_errors_partial_shows_failed_jobs(self, admin_client, session: AsyncSession):
        ...
        await create_thinker(session, name="Err Thinker", slug="err-thinker")
        source = await create_source(session, name="Err Source", url="https://example.com/err-feed.xml")
        await create_job(
            session,
