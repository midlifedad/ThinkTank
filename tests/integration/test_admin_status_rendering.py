...
class TestQueueDepthDoneColumn:
    ...
    async def test_done_jobs_counted_in_done_column(
        ...
        # reasonable proxy.
        assert ">1<" in body, (
            "Done job count did not render; template still only counts 'completed'"
        )
...
class TestThinkerContentStatusRendering:
    ...
    async def test_done_renders_healthy(
        self, admin_client: AsyncClient, session: AsyncSession
    ):
        thinker_id, _ = await self._setup_content(session, "done")
        resp = await admin_client.get(
            f"/admin/thinkers/{thinker_id}/partials/content"
        )
        assert resp.status_code == 200
        assert 'class="healthy"' in resp.text

    async def test_cataloged_renders_healthy(
        self, admin_client: AsyncClient, session: AsyncSession
    ):
    ...
    async def test_pending_renders_warning(
        self, admin_client: AsyncClient, session: AsyncSession
    ):
    ...
    async def test_error_renders_error(
        self, admin_client: AsyncClient, session: AsyncSession
    ):
    ...
    async def test_skipped_renders_error(
        self, admin_client: AsyncClient, session: AsyncSession
    ):
...
class TestSourceEpisodesStatusRendering:
    ...
    async def test_done_renders_healthy(
        ...
        resp = await admin_client.get(
            f"/admin/sources/{source_id}/partials/episodes"
        )
        assert resp.status_code == 200
        assert 'class="healthy"' in resp.text
