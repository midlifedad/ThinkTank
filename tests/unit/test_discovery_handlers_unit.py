...
Tests that all 3 discovery handlers are registered in the handler registry
...
class TestDiscoveryHandlerRegistration:
    """All 3 Phase 6 handlers must be registered in the handler registry."""
    ...
    def test_discover_guests_listennotes_registered(self):
        handler = get_handler("discover_guests_listennotes")
        assert handler is not None, "discover_guests_listennotes not registered"
...
class TestDiscoveryHandlerProtocol:
    ...
    def test_discover_guests_listennotes_is_async(self):
        handler = get_handler("discover_guests_listennotes")
        assert handler is not None
        assert inspect.iscoroutinefunction(handler)
    ...
    def test_discover_guests_listennotes_signature(self):
        handler = get_handler("discover_guests_listennotes")
        assert handler is not None
        sig = inspect.signature(handler)
        params = list(sig.parameters.keys())
