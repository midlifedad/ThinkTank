...
class TestSecondsUntilNextUtcHour:
    ...
    def test_before_target_hour_same_day(self):
        ...
        fake_now = datetime(2026, 3, 9, 3, 30, 0, tzinfo=timezone.utc)
    ...
    def test_after_target_hour_wraps_to_next_day(self):
        ...
        fake_now = datetime(2026, 3, 9, 8, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_utc_hour(7)
    ...
    def test_exactly_at_target_hour_wraps_to_next_day(self):
        ...
        fake_now = datetime(2026, 3, 9, 7, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_utc_hour(7)
    ...
    def test_minutes_before_target(self):
        ...
        fake_now = datetime(2026, 3, 9, 6, 45, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_utc_hour(7)
    ...
    def test_midnight_target(self):
        ...
        fake_now = datetime(2026, 3, 9, 23, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
    ...
    def test_result_is_positive_float(self):
        ...
        fake_now = datetime(2026, 3, 9, 12, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_utc_hour(7)
...
class TestSecondsUntilNextMondayUtc:
    ...
    def test_monday_before_target_hour(self):
        ...
        fake_now = datetime(2026, 3, 9, 3, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_monday_utc(7)
    ...
    def test_sunday_to_monday(self):
        ...
        fake_now = datetime(2026, 3, 15, 20, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_monday_utc(7)
    ...
    def test_friday_to_monday(self):
        ...
        fake_now = datetime(2026, 3, 13, 10, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_monday_utc(7)
    ...
    def test_result_is_positive_float(self):
        """Return value is always a positive float."""
        fake_now = datetime(2026, 3, 11, 12, 0, 0, tzinfo=timezone.utc)
        with patch("src.thinktank.llm.time_utils._utc_now", return_value=fake_now):
            result = seconds_until_next_monday_utc(7)
        assert isinstance(result, float)
        assert result > 0
